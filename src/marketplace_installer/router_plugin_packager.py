...
from marketplace_installer.router_plugin_packager_mcp_normalization import (
    validate_mcp_launch_policy as _validate_mcp_launch_policy_impl,
)
from marketplace_installer.router_plugin_packager_mcp import (
...
class McpPackaging:
    ...
    launch_contract: McpLaunchContract
...
def _normalize_mcp_packaging(
    ...
    return _normalize_mcp_packaging_impl(
        ...
        ensure_string=_ensure_string,
        normalize_mcp_launch_contract_fn=_normalize_mcp_launch_contract,
...
def _normalize_mcp_launch_contract(  # noqa: C901
    payload: dict[str, Any],
) -> McpLaunchContract:
    return _normalize_mcp_launch_contract_impl(
        payload,
        ensure_string=_ensure_string,
        require_string_list_fn=_require_string_list,
        normalize_mcp_environment_fn=_normalize_mcp_environment,
        validate_mcp_launch_policy_fn=_validate_mcp_launch_policy,
        mcp_launch_contract_factory=McpLaunchContract,
    )


def _normalize_mcp_environment(
    value: Any, *, field: str
) -> tuple[tuple[str, str], ...]:
    return _normalize_mcp_environment_impl(
        value, field=field, canonical_json_bytes=_canonical_json_bytes
    )
...
def _validate_mcp_launch_policy(contract: McpLaunchContract) -> None:
    _validate_mcp_launch_policy_impl(contract)
...
def _discover_source_root(repository_root: Path) -> tuple[Path, str]:
    return _discover_source_root_impl(
        repository_root, has_hidden_path_segment=_has_hidden_path_segment
    )
...
def _validate_mcp_descriptor_round_trip(contract: McpLaunchContract) -> None:
