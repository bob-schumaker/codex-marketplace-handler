...
ASSEMBLY_RECEIPT_NAME = ".marketplace-assembly-receipt.json"
ASSEMBLY_RECEIPT_FORMAT = "marketplace-assembly-receipt-v1"
...
def _validate_generated_plugin_payload(
    plugin_root: Path,
) -> tuple[str, str, dict[str, Any]]:
    """Validate generated bytes that may be copied without their source checkout."""
    ...
    return plugin_name, _sha256(receipt_path), receipt
...
def _validate_generated_plugin_root(
    ...
    plugin_name, receipt_digest, receipt = _validate_generated_plugin_payload(
        plugin_root
    )
...
def _assembly_receipt(
    *, marketplace_name: str, plugin_name: str, plugin_root: Path, manifest_path: Path
) -> dict[str, str]:
    return {
        "format": ASSEMBLY_RECEIPT_FORMAT,
        "marketplace_name": marketplace_name,
        "plugin_name": plugin_name,
        "plugin_tree_digest": _tree_digest(plugin_root),
        "marketplace_manifest_digest": _sha256(manifest_path),
    }


def _validate_portable_marketplace_payload(
    root: Path, *, allow_package_marker: bool = False
) -> tuple[str, str]:
    """Validate a self-contained assembled marketplace without source-tree checks."""

    _validate_regular_tree(root)
    allowed = {".agents", "plugins", ASSEMBLY_RECEIPT_NAME}
    actual = {path.name for path in root.iterdir()}
    if allow_package_marker:
        marker = root / "__init__.py"
        if marker.is_symlink() or not marker.is_file():
            raise MarketplacePublishError("resource package marker is invalid")
        actual.remove("__init__.py")
        bytecode_cache = root / "__pycache__"
        if bytecode_cache.exists():
            if bytecode_cache.is_symlink() or not bytecode_cache.is_dir():
                raise MarketplacePublishError("resource package cache is invalid")
            actual.remove("__pycache__")
    if actual != allowed:
        raise MarketplacePublishError("canonical marketplace root has invalid contents")
    receipt_path = root / ASSEMBLY_RECEIPT_NAME
    receipt = read_json_object(receipt_path)
    expected_keys = {
        "format",
        "marketplace_name",
        "plugin_name",
        "plugin_tree_digest",
        "marketplace_manifest_digest",
    }
    if (
        set(receipt) != expected_keys
        or receipt.get("format") != ASSEMBLY_RECEIPT_FORMAT
    ):
        raise MarketplacePublishError("assembly receipt is invalid")
    marketplace_name = receipt.get("marketplace_name")
    ...
    if (
        ...
        raise MarketplacePublishError("assembly receipt is invalid")
    manifest_path = source_marketplace_manifest_path(root)
    manifest = load_marketplace_manifest(root)
    plugins = validated_plugin_map(root, manifest)
    if set(plugins) != {plugin_name} or manifest.get("name") != marketplace_name:
        raise MarketplacePublishError("canonical marketplace manifest is invalid")
    validated_name, _receipt_digest, _router_receipt = (
        _validate_generated_plugin_payload(root / "plugins" / plugin_name)
    )
    if validated_name != plugin_name:
        raise MarketplacePublishError(
            "canonical marketplace plugin identity is invalid"
        )
    expected = _assembly_receipt(
        marketplace_name=marketplace_name,
        plugin_name=plugin_name,
    ...
    if receipt != expected:
        raise MarketplacePublishError(
            "assembly receipt does not match marketplace payload"
        )
    return marketplace_name, plugin_name


def assemble_generated_plugin(
    *, plugin_root: Path, assembly_root: Path, marketplace_name: str
) -> dict[str, Any]:
    """Create a self-contained one-plugin marketplace from strict v3 output."""

    if not isinstance(marketplace_name, str) or not marketplace_name:
        raise MarketplacePublishError("marketplace name is required")
    plugin_root = plugin_root.absolute()
    assembly_root = assembly_root.absolute()
    plugin_name, _receipt_digest, _mcp_authority, _source_inventory = (
        _validate_generated_plugin_root(plugin_root)
    )
    if assembly_root.exists():
        if assembly_root.is_symlink() or not assembly_root.is_dir():
            raise MarketplacePublishError(f"assembly root is invalid: {assembly_root}")
        if any(assembly_root.iterdir()):
            raise MarketplacePublishError(
                f"assembly root must be empty: {assembly_root}"
            )
    else:
        assembly_root.mkdir(parents=True)
    destination = assembly_root / "plugins" / plugin_name
    destination.parent.mkdir(parents=True)
    shutil.copytree(plugin_root, destination, symlinks=False)
    manifest_path = source_marketplace_manifest_path(assembly_root)
    _write_json_atomic(
        manifest_path,
        {
    ...
    _write_json_atomic(
        assembly_root / ASSEMBLY_RECEIPT_NAME,
        _assembly_receipt(
            marketplace_name=marketplace_name,
            plugin_name=plugin_name,
    ...
    _validate_portable_marketplace_payload(assembly_root)
    return {
...
def stage_marketplace_payload(
    canonical_root: Path, destination_resources: Path
) -> dict[str, Any]:
    """Atomically replace package resources with a validated canonical payload."""

    canonical_root = canonical_root.absolute()
    destination_resources = destination_resources.absolute()
    _validate_portable_marketplace_payload(canonical_root)
    if destination_resources.exists() and destination_resources.is_symlink():
        raise MarketplacePublishError(
            f"destination resources are unsafe: {destination_resources}"
        )
    if destination_resources.exists() and not destination_resources.is_dir():
        raise MarketplacePublishError(
            f"destination resources are invalid: {destination_resources}"
        )
    if destination_resources.exists():
        _validate_regular_tree(destination_resources)
    destination_resources.parent.mkdir(parents=True, exist_ok=True)
    stage_root = Path(
        tempfile.mkdtemp(
            prefix=f".{destination_resources.name}.stage-",
            dir=destination_resources.parent,
        )
    )
    staged_resources = stage_root / destination_resources.name
    backup_root = stage_root.with_name(
        f".{destination_resources.name}.backup-{uuid.uuid4().hex}"
    )
    try:
        staged_resources.mkdir()
        marker = destination_resources / "__init__.py"
        if marker.is_file() and not marker.is_symlink():
            shutil.copy2(marker, staged_resources / "__init__.py")
        for source in canonical_root.iterdir():
            target = staged_resources / source.name
            if source.is_dir():
                shutil.copytree(source, target, symlinks=False)
            else:
                shutil.copy2(source, target)
        if destination_resources.exists():
            os.replace(destination_resources, backup_root)
        os.replace(staged_resources, destination_resources)
        if backup_root.exists():
            shutil.rmtree(backup_root)
    except Exception:
        if backup_root.exists() and not destination_resources.exists():
            os.replace(backup_root, destination_resources)
        raise
    finally:
        if stage_root.exists():
            shutil.rmtree(stage_root)
    return {
        "destination_resources": str(destination_resources),
        "payload_digest": _tree_digest(canonical_root),
    }
...
def _publication_result(
    *,
    ...
    return {
        ...
        "unchanged": sorted(unchanged or []),
        "conflicts": sorted(conflicts or []),
    }


def _validate_operational_marketplace_target(
    root: Path,
) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    if root.is_symlink() or not root.is_dir():
        raise MarketplacePublishError(f"marketplace target is invalid: {root}")
    _validate_regular_tree(root)
    allowed = {
        ".agents",
        "plugins",
        ...
        RECEIPT_ROOT_NAME,
        f"{TRANSACTION_SUFFIX}.lock",
    }
    unexpected = {path.name for path in root.iterdir()} - allowed
    if unexpected:
        raise MarketplacePublishError("marketplace target has unsafe root entries")
    manifest = load_marketplace_manifest(root)
    plugins = validated_plugin_map(root, manifest)
    for plugin_name in plugins:
        _validate_regular_tree(root / "plugins" / plugin_name)
    return manifest, plugins


def _copy_canonical_root(canonical_root: Path, destination: Path) -> None:
    destination.mkdir()
    for source in canonical_root.iterdir():
        if source.name in {"__init__.py", "__pycache__"}:
            continue
...
def _install_new_canonical_marketplace(canonical_root: Path, target_root: Path) -> None:
    target_root.parent.mkdir(parents=True, exist_ok=True)
    stage_parent = Path(
        tempfile.mkdtemp(prefix=f".{target_root.name}.stage-", dir=target_root.parent)
    )
    staged = stage_parent / target_root.name
    try:
        _copy_canonical_root(canonical_root, staged)
...
def _replace_canonical_plugin(
    *,
    ...
    """Replace one plugin and its manifest entry, restoring both on failure."""

    source_entry = validated_plugin_map(
        canonical_root, load_marketplace_manifest(canonical_root)
    )[plugin_name]
    existing_entries = manifest["plugins"]
    merged_entries = [
        source_entry if entry.get("name") == plugin_name else entry
        for entry in existing_entries
        if isinstance(entry, dict)
    ]
    if not any(entry.get("name") == plugin_name for entry in existing_entries):
        merged_entries.append(source_entry)
    merged_manifest = {**manifest, "plugins": merged_entries}
    target_plugin = target_root / "plugins" / plugin_name
    target_manifest = source_marketplace_manifest_path(target_root)
    previous_manifest = target_manifest.read_bytes()
    stage_parent = Path(
        tempfile.mkdtemp(prefix=".marketplace-plugin-", dir=target_root)
    )
    staged_plugin = stage_parent / plugin_name
    backup_plugin = stage_parent / "backup"
    shutil.copytree(
        canonical_root / "plugins" / plugin_name, staged_plugin, symlinks=False
    )
    replaced_plugin = False
    try:
        target_plugin.parent.mkdir(parents=True, exist_ok=True)
        if target_plugin.exists():
            os.replace(target_plugin, backup_plugin)
        os.replace(staged_plugin, target_plugin)
        replaced_plugin = True
        _write_json_atomic(target_manifest, merged_manifest)
    except Exception:
        if replaced_plugin and target_plugin.exists():
            shutil.rmtree(target_plugin)
        if backup_plugin.exists():
            os.replace(backup_plugin, target_plugin)
        target_manifest.write_bytes(previous_manifest)
        raise
    finally:
        if stage_parent.exists():
            shutil.rmtree(stage_parent)
...
def publish_embedded_generated_marketplace(
    ...
    *,
    ...
    """Publish a portable canonical payload bundled in an installed package."""

    try:
        resource_files = resources.files(resource_package)
    except (ModuleNotFoundError, TypeError) as error:
        raise MarketplacePublishError(
            f"embedded marketplace resources are unavailable: {resource_package}"
        ) from error
    with resources.as_file(resource_files) as materialized:
        canonical_root = Path(materialized)
        marketplace_name, plugin_name = _validate_portable_marketplace_payload(
            canonical_root, allow_package_marker=True
        )
        target_root = (
            (home or Path.home()) / ".codex" / "local-marketplaces" / marketplace_name
        )
        if not target_root.exists():
            result = _publication_result(
                status="preview" if dry_run else "installed",
                marketplace=marketplace_name,
                dry_run=dry_run,
                added=[plugin_name],
            )
            if not dry_run:
                _install_new_canonical_marketplace(canonical_root, target_root)
            return result
        if target_root.is_symlink():
            if dry_run:
                return _publication_result(
                    ...
                    marketplace=marketplace_name,
                    dry_run=True,
                    conflicts=["marketplace"],
                )
            raise MarketplacePublishError("marketplace target is invalid")
        try:
            _recover_interrupted_transaction(target_root)
            installed_manifest, installed_plugins = (
                _validate_operational_marketplace_target(target_root)
            )
            if installed_manifest.get("name") != marketplace_name:
                raise MarketplacePublishError("marketplace target has a different name")
        ...
        existing_entry = installed_plugins.get(plugin_name)
        target_plugin = target_root / "plugins" / plugin_name
        if existing_entry is None:
            ...
            return result
        equal = _tree_digest(target_plugin) == _tree_digest(
            canonical_root / "plugins" / plugin_name
        ) and _canonical_json_bytes(existing_entry) == _canonical_json_bytes(
            source_entry
        )
        if equal:
            return _publication_result(
        ...
        if not force:
            raise MarketplacePublishError(
                f"marketplace plugin conflicts: {plugin_name}"
            )
        _replace_canonical_plugin(
            canonical_root=canonical_root,
            target_root=target_root,
